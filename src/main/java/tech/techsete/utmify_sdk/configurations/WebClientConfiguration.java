...
/**
 * Configuração do cliente HTTP reativo para o SDK Utmify.
 * <p>
 * Esta classe fornece a configuração necessária para a comunicação com a API da Utmify,
 * criando e configurando uma instância de {@link WebClient} do Spring WebFlux.
 * </p>
 *
 * O WebClient é configurado com:
 * <ul>
 *   <li>URL base da API Utmify</li>
 ...
 * com os endpoints da API Utmify, fornecendo uma interface reativa para
 * requisições HTTP.
 * </p>
 */

@Slf4j
@Configuration("UtmifySDKWebClientConfiguration")
public class WebClientConfiguration {

    /**
     * Cria e configura um {@link WebClient} para comunicação com a API Utmify.
     * <p>
     * Este bean cria uma instância de WebClient configurada especificamente para
     * interagir com a API da Utmify. A configuração inclui:
     * </p>
     * <ul>
     *   <li>URL base da API: https://api.utmify.com.br</li>
     ...
     * requisições HTTP reativas à API da Utmify.
     * </p>
     *
     * @return uma instância configurada de {@link WebClient}
     */
    @Bean(name = "UtmifyWebClient")
    public WebClient UtmifyWebClient() {

        return WebClient.builder()
                .baseUrl("https://api.utmify.com.br")
    ...
    private Mono<ClientResponse> handleErrors(ClientResponse response) {
        if (response.statusCode().is4xxClientError() || response.statusCode().is5xxServerError()) {
            return response.bodyToMono(String.class)
                    .flatMap(errorBody -> {
                        log.error("Erro HTTP {} na chamada à API Utmify: {}", response.statusCode().value(), errorBody);
                        return Mono.error(new RuntimeException("Erro ao chamar API Utmify: " + errorBody));
                    });
        }
